...
func main() {
	dropSand := func(world map[position]int) bool {
		x, y := 500, 0

		for y < 1000 {
			if world[pos(x, y+1)] == air {
				y++
			} else if world[pos(x-1, y+1)] == air {
				x--
				y++
			} else if world[pos(x+1, y+1)] == air {
				x++
...

	{
		...
		for x := -1000; x <= 2000; x++ {
			world[pos(x, maxY+2)] = wall
		}

		n := 1 // Start at 1 because our algorithm will not actually  fill 500,0.
		for dropSand(world) {
			n++
		}
...
func makeWorld(input [][][2]int) map[position]int {
	world := make(map[position]int)

	makeWall := func(from, to position) {
		if from.x == to.x {
			x := from.x
			y1, y2 := from.y, to.y
			if y2 < y1 {
	...
	for _, lineStrip := range input {
		for i := 1; i < len(lineStrip); i++ {
			makeWall(
				toPos(lineStrip[i-1]),
				toPos(lineStrip[i]),
			)
