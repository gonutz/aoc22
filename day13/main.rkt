...
"part 1"
(define right-order -1)
(define wrong-order 1)
(define unknown-order 0)
(define (int-order a b) (sgn (- a b)))
...
(define (order a b)
  (cond ((and (integer? a) (integer? b))
        ...
        (else
         (let ([first-order (order (car a) (car b))])
           (if (equal? first-order unknown-order)
               (order (cdr a) (cdr b))
               first-order)))))

(define (in-right-order a b) (equal? (order a b) right-order))

(define (right-order-sum list index)
  (if (null? list) 0
      (+
       (if (in-right-order (car list) (cadr list)) index 0)
       (right-order-sum (cddr list) (+ index 1)))))

(right-order-sum input 1)

"part 2"
(define input-with-dividers (cons '((2)) (cons '((6)) input)))
(define sorted (sort input-with-dividers in-right-order))

(define (locate list item)
  (define (locate-index list item index)
    (if (equal? (car list) item) index (locate-index (cdr list) item (+ index 1))))
  (locate-index list item 1)
  )

(* (locate sorted '((2))) (locate sorted '((6))))
